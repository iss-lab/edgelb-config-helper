// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"github.com/go-openapi/errors"
	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
	"github.com/go-openapi/validate"
)

// PoolTemplate Specifies the configuration of a pool template
//
// swagger:model PoolTemplate
type PoolTemplate struct {

	// Name of the pool template
	// Required: true
	// Pattern: ^[a-zA-Z0-9][a-zA-Z0-9-]+$
	Name *string `json:"name"`
}

// Validate validates this pool template
func (m *PoolTemplate) Validate(formats strfmt.Registry) error {
	var res []error

	if err := m.validateName(formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (m *PoolTemplate) validateName(formats strfmt.Registry) error {

	if err := validate.Required("name", "body", m.Name); err != nil {
		return err
	}

	if err := validate.Pattern("name", "body", string(*m.Name), `^[a-zA-Z0-9][a-zA-Z0-9-]+$`); err != nil {
		return err
	}

	return nil
}

// MarshalBinary interface implementation
func (m *PoolTemplate) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *PoolTemplate) UnmarshalBinary(b []byte) error {
	var res PoolTemplate
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
